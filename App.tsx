...
const App: React.FC = () => {
  ...
  const caddyPosition = useMemo(() => {
    ...
    const gridWidth = currentLevel.grid[0].length * 40;
    const gridHeight = currentLevel.grid.length * 40;
    
    const isPortrait = height > width && width < 640;
    let scaledWidth = gridWidth;
    ...
    if (isPortrait) {
      ...
      let baseScale = Math.min(availableW / gridWidth, availableH / gridHeight);
      baseScale *= 1.25;
      scaledWidth = gridWidth * baseScale;
    }
    
    const spaceSide = (width - scaledWidth) / 2;
    
    if (spaceSide > 420) {
      return 'right';
    } else if (spaceSide > 320 && gridHeight > gridWidth) {
      return 'right';
    }
  ...
  const renderHUD = () => {
    const positionClasses = caddyPosition === 'right'
      ? "absolute top-1/2 -translate-y-1/2 right-8 z-40 w-full max-w-sm select-none px-4"
      : "absolute bottom-[200px] lg:bottom-[120px] left-1/2 -translate-x-1/2 z-40 w-full max-w-md select-none px-4";
    ...
    if (gameState.state === 'LEVEL_COMPLETE') {
        ...
        return (
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-xl p-[55px]">
                <div className="bg-[#131313] text-white px-[30px] lg:px-10 py-10 lg:py-[50px] rounded-3xl text-center w-full border border-white/10 animate-in fade-in zoom-in duration-200 flex flex-col items-center min-w-xs max-w-md">
