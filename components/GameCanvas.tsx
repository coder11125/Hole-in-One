...
const GameCanvas: React.FC<GameCanvasProps> = ({ level, onStroke, onHole }) => {
  ...
  const calculateLayout = useCallback((width: number, height: number) => {
    ...
    if (isPortrait) {
      ...
      // In landscape mode: fit within available space with slight padding
      let availableW = width - 40;
      let availableH = height - 40; 
      
      baseScale = Math.min(availableW / gridWidth, availableH / gridHeight);
    }

    // General zoom in requested by user
    baseScale *= 1.25;
  ...
  const render = useCallback(() => {
    ...
    // Fill the green with a traditional golf green color
    ...
    decorationsRef.current.forEach(d => {
        if (d.type === 'ANDROID') {
            // Draw a generic circular bumper
            ctx.save();
